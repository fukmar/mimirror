...
public void LiberarMesaCombinada (Mesa mesacombinada)
{
	...
	public List<Mesa> getMesasOcupadas()
	{
		Session session=sf.openSession();
		List<Mesa> listaM=new ArrayList<Mesa>();
		List<MesaEntity> resu=session.createQuery("from MesaEntity m where m.estado=1 and m.combinada<>2").list();
		for(MesaEntity m:resu)
		{
			listaM.add(m.toNegocio()); 
		}
	...
	public List<Mesa> getMesasFacturables()
	{
		Session session=sf.openSession();
		List<Mesa> listaM=new ArrayList<Mesa>();
		List<ComandaEntity> resu=session.createQuery("from ComandaEntity c where c.estado=?").setParameter(0, Estado.EnProceso).list();
		for(ComandaEntity c:resu)
		{
			Mesa m=MesaDAO.getInstance().getMesaN(c.getMesa().getCodMesa());
			if(!listaM.contains(m))
			listaM.add(m);
		}
		session.close();
		return listaM;
	...
	public boolean validateMesasLibresbyCantidad(int cantidadpersonas)
	{
		Session session=sf.openSession();
		List<Mesa> listaM=new ArrayList<Mesa>();
		List<MesaEntity> resu=session.createQuery("from MesaEntity m where m.estado=0 and m.capacidad>? or m.capacidad=?").setInteger(0, cantidadpersonas).setInteger(1, cantidadpersonas).list();
		for(MesaEntity m:resu)
		{
			listaM.add(m.toNegocio());
		}
