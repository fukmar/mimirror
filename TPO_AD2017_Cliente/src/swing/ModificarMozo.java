...
public class ModificarMozo extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ModificarMozo frame = new ModificarMozo();
	...
	public ModificarMozo() {
		...
		textFieldApellido.setBounds(391, 377, 260, 35);
		...
		btnModificarMozo.setFont(new Font("Tahoma", Font.PLAIN, 18));
		btnModificarMozo.setBounds(142, 615, 204, 48);
		...
		btnBuscar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					List<MozoDTO> mozos= BusinessDelegate.getInstance().mostrarMozos();
					for(MozoDTO mozo: mozos) {
						if(mozo.getDni()==Integer.parseInt(textFieldIngresar.getText())) {
							textFieldDocumento.setText(mozo.getDni().toString());
							textFieldApellido.setText(mozo.getApellido());
							textFieldNombre.setText(mozo.getNombre());
							textFieldComision.setText(mozo.getProcentajeComision().toString());
							textFieldDocumento.setEditable(true);
							textFieldApellido.setEditable(true);
							textFieldNombre.setEditable(true);
							textFieldComision.setEditable(true);
							textFieldIngresar.setEditable(false);
							//BusinessDelegate.
						}
					}
				} catch (RemoteException | MozoException e1) {
